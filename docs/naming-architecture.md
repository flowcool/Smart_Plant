...
**Status: REVIEWED DECISION RECORD (rev 3). No implementation or live change.**
...
files, ESPHome 2026.7.4, Home Assistant 2026.8.1, and the HA registries. The
...
The **entity layer has no independent name**. Every one of the 12 exposed
...
The e-paper "normal" page is a per-device PNG (`page_1_background`, `platform:
file`) generated by the **Label-maker** tool. That PNG bakes in the plant name
*and* the recommended-range arcs (`parameters.*.rel_range`). Watering thresholds
were since revised and moved into Home Assistant, so the baked arcs now diverge
from the live HA thresholds, and any name change still requires regenerating an
opaque NAS-only image. Two problems, one root: static art carrying data. The
per-plant Label-maker JSON sources are **not in the repo or on the NAS** — only
the final PNGs survive, so the labels are currently **not reproducible**.
...
**object_id (`device_name` generator, currently OFF):**
```cpp
...
### 2.1 Current device display and MAC suffixing (load-bearing)

Each of the eight production device YAML files currently defines an explicit
...
The human label is therefore duplicated today; `display_name` does not feed the
other two fields automatically. The target model removes that duplication.
...
### 3.2 Firmware changes (spec for `infra-zdxz.2`, not applied here)
...
1. The **deterministic FUTURE mapping**, keyed by `(device, component_type,
   function)` → target clean `entity_id` (`<node_name>_<function>`) and the fact
   that its `unique_id` becomes `<mac>-<component>-<fnv1(function-name)>`.
...
new_discovery_topic, new_entity_id, new_unique_id
```
...
| `subtitle.value` | botanical name | ← `botanical_name` (descriptive metadata) |
| `parameters.*.rel_range` | recommended-range **arcs** (moisture/light/temp/humidity) | ← thresholds, **HA-authoritative** |

**The name half is settled:** the normal page renders `display_name` + botanical
...
| 6 | `esphome.friendly_name` | explicit human literal in all 8 live YAMLs; MAC appended on the 6 suffix-enabled devices | `${display_name}` (no MAC, suffix false) |
...
`node = <configured_name>` (e.g. `cyperus_papyrus_54a9b2`). Future `entity_id` =
`<domain>.<node>_<function>`. AS-IS `entity_id` is the HA-frozen ugly form
...
| G3 | e-paper font glyph coverage — Audiowide has the glyphs, but the default `GF_Latin_Kernel` glyphset **excludes `œ` and accents**; declare `GF_Latin_Core` and keep a layout/render gate on the 8 strings | Implementation gate — owned by `infra-zdxz.3` |
...
## 8. Sequence (not executed here)

1. `infra-zdxz.1`: keep `naming.md` accurate for current production and this
   document as the locked target/runbook contract.
2. `infra-zdxz.2`: firmware refactor — uniform explicit identity (all 8,
...
3. `infra-zdxz.3`: extract the one-off HA threshold snapshot, generate name-free
   e-paper art/arcs, render names dynamically, and declare `GF_Latin_Core`.
4. `infra-kl21` (HA): generate the 96-row correspondence table, then perform the
...
No step here changes firmware, NAS, or HA state. Execution requires the owning
Beads issues, their rollback prerequisites, and Florent's explicit authorization
for each live write, compile, upload, or flash.
